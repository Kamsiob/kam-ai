...
**Steps 4 to 6.** All twenty frames read individually, both themes, then the listing frames
...
- **Workbench cannot be seeded like a chat, but it does make conversations.** Its input is the
  `workbench.input` setting rather than a chat, and its send button is nowhere near a chat
  composer's, so `say.sh` types into the box and its send tap lands on nothing. The first seeding
  run reported eight of nine with the eighth having produced no conversation at all.
  **Corrected later the same day:** running an actual change from the Workbench screen *does*
  create a conversation, and it appears in the chat list with the gold Workbench dot. An earlier
  version of this file said Workbench never lands in the list, and that was wrong. It is still out
  of `seed_chats.sh`, because the way to drive it is the chips and not the composer.
...
6. **Upload the eight phone screenshots.** `store-assets/phone`, 1080x2224, generated by
   `tools/make_phone_shots.sh`, which is all eight slots Play allows. Nothing has ever been
   uploaded to the Console, so no superseded asset can be lingering there.
