...
class RestatementRetryTest {
    ...
    fun anEmptyMessageGetsNoNudge() {
        ...
        assertThat(RestatementRetry.instruction("", Mode.GENERAL)).isNull()
        assertThat(RestatementRetry.instruction("   ", Mode.GENERAL)).isNull()
    }

    @Test
    fun theNudgeAsksForSomethingRatherThanForbiddingSomething() {
        val text = RestatementRetry.instruction("Bread needs a hot oven, around 230C.", Mode.GENERAL)!!
    ...
    @Test
    fun theModesWithTheirOwnReplyShapeAreLeftAlone() {
        // Found by measuring. Applied to Logic Partner, this instruction was
        // obeyed exactly and the mode collapsed into one bland line, because no
        // single additive sentence can be the argument at its strongest, its
        // weakest link and the question that would settle it. Brainstorm has the
        // same problem: it owes one thing it is doing and one question.
        listOf(Mode.LOGIC, Mode.BRAINSTORM, Mode.BENCH, Mode.DISCOVER).forEach { mode ->
            assertThat(RestatementRetry.instruction("Bread needs a hot oven.", mode)).isNull()
        }
    }

    @Test
    fun theModesWhereAOneLineReplyIsRightStillGetIt() {
        listOf(Mode.GENERAL, Mode.OVERLAY).forEach { mode ->
            assertThat(RestatementRetry.instruction("Bread needs a hot oven.", mode)).isNotNull()
        }
    }
    ...
    fun theNudgeCarriesNoContentToCopy() {
        ...
        val text = RestatementRetry.instruction("Water boils at 100 degrees Celsius.", Mode.GENERAL)!!
